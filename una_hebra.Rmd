...
filtFs <- file.path(path, "filtered.1", paste0(sample.names, "_F_filt.fastq.gz"))#criteria for the outing
...
```{r}
out <- filterAndTrim(fnFs, filtFs, truncLen=165, #first is the path of the forward fastq file, 
              ...
              maxN=0, maxEE=2, truncQ=2, rm.phix=TRUE, #maxN: after being truncated, any sequences with a higher Ns than what it is stated
                                                            #won't be used
...
#out1<-the trims are on the 155
#out2<-the trims are on the 175
#out3<-the trims are on the 185
#out4<-the trims are on the 165
...
```{r}
seqtab.nochim <- makeSequenceTable(dadaFs)#This function constructs a sequence table (analogous to an OTU table) from the provided list of samples.
dim(seqtab)
```
...
track <- cbind(out, sapply(dadaFs, getN), rowSums(seqtab.nochim))#this should get us the reads and the unique sequences we had at the beginning and at each step; since it looks like it hasn't dropped o much, it is fine
...
save(ps1, file="~/R/Corroborar_Roberto_1/ps1_F.RData")
save(ps2, file="~/R/Corroborar_Roberto_1/ps2_F.RData")
save(ps3, file="~/R/Corroborar_Roberto_1/ps3_F.RData")
save(ps4, file="~/R/Corroborar_Roberto_1/ps4_F.RData")
```
